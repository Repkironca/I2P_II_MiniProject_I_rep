...
long long int match(TokenSet token);
// Get the next token
void advance(void);
...
long long int sbcount = 0;
Symbol table[TBLSIZE];

// Initialize the symbol table with builtin variables
void initTable(void);
// Get the value of a variable
long long int getval(char *str);
// Set the value of a variable
long long int setval(char *str, long long int val);
...
long long int calculateConstant(BTNode *root);
long long int evaluateTree(BTNode *root);
long long int get_reg(void);
void free_reg(long long int index);
...
long long int isVarName (char ch) {
...
long long int setval(char *str, int val) {
    long long int i = 0;
...
long long int isBiOpr(BTNode *root){
...
BTNode *foldConstant(BTNode *root){
    ...
    if (isBiOpr(root) && !hasVariable(root)){
        long long int val = calculateConstant(root);
        char buf[MAXLEN];
...
void statement(void) {
    ...
    if (match(ENDFILE)) {
        ...
        if (match(END)) {
            ...
            long long int final_reg = evaluateTree(retp);
            free_reg(final_reg);
...
long long int reg_status[8] = {0}; // 表示有哪些 register 是空的，0 表示空著
...
long long int hasVariable(BTNode *root) {
...
long long int calculateConstant(BTNode *root) {
    long long int retval = 0, lv = 0, rv = 0;
    ...
    if (root != NULL) {
        switch (root->data) {
            case ID:
                ...
                long long int addr = getval(root->lexeme); // 原本在 memory 的位置
                printf("MOV r%d [%d]\n", retval, addr);
                break;
            case INT:
                retval = get_reg(); // 拿一個新的 register
            ...
            case ASSIGN: // 例如 x = 7
                ...
                long long int left_addr = setval(root->left->lexeme, 0);
                printf("MOV [%d] r%d\n", left_addr, rv); // 存回去記憶體
