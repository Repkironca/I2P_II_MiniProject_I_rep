...
int get_reg(TokenSet type, char *name);
...
char reg[8][MAXLEN] = {"", "", "", "", "", "", "", ""}; // 記憶體裡面放了誰

// 確認一個東西是否在 reg 中
int locate_reg(char *name){
    for (int i = 0; i <= 7; i++){
        if (strcmp(reg[i], name) == 0 && !reg_status[i]){ // 如果 reg 中有它，而且它能用
            return i;
        }
    }
    return -1; // 找不到是 return -1
}

// 找到一個空的 register
int get_reg(TokenSet type, char *name){
    int hit = locate_reg(name);
    if (hit != -1){
        reg_status[hit] = 1;
        return hit;
    }
    ...
    int ret = -1;
    // 先看有沒有專屬格子，沒有的話就當正常人
    if (strcmp(name, "x") == 0 && !reg_status[0]) ret = 0;
    else if (strcmp(name, "y") == 0 && !reg_status[1]) ret = 1;
    else if (strcmp(name, "z") == 0 && !reg_status[2]) ret = 2;
    else if (strcmp(name, "1") == 0 && !reg_status[3]) ret = 3;
    else {
        int order[8] = {4, 5, 6, 7, 0, 1, 2, 3};
        for (int i = 0; i <= 7; i++){
            if (!reg_status[order[i]]){
                ret = order[i];
                break;
            }
        }
...
// 更新某格的名稱，並抹掉所有同名的
void update_reg(int index, char *name){
    strcpy(reg[index], name);
    for (int i = 0; i <= 7; i++){
        if (i != index&& strcmp(reg[i], name) == 0){
            strcpy(reg[i], "");
        }
    }
}
...
int evaluateTree(BTNode *root) {
    ...
    if (root != NULL) {
        switch (root->data) {
            case ID:
                retval = get_reg(root->data, root->lexeme);
                break;
            case INT:
                retval = get_reg(root->data, root->lexeme);
                break;
            case ASSIGN: // 例如 x = 7
                ...
                printf("MOV [%d] r%d\n", left_addr, rv); // 存回去記憶體
                update_reg(rv, root->left->lexeme); // 現在右小孩變成快取
                retval = rv;
                break;
            case ADDSUB:
            case MULDIV:
            ...
            case XOR:
                ...
                // 處理記憶體
                strcpy(reg[lv], ""); // 左小孩不再是本來的樣子了，拔標籤
                if (lv != rv) free_reg(rv); // 避免 ADD r0 r0 這種 case
...
r[1] = -9
r[2] = -169
Total clock cycles are 2090

r[0] = -1
r[1] = 200
r[2] = 10
Total clock cycles are 8960
...
r[1] = 0
r[2] = 0
Total clock cycles are 830

r[0] = 0
r[1] = 0
r[2] = 4
Total clock cycles are 2280
*/